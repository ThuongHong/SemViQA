...
from transformers import default_data_collator, AutoModelForQuestionAnswering, AutoConfig
...
def main(args):
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            batch = {k: v for k, v in batch.items() if k not in ["Tagging","token_type_ids"]}
            outputs = model(**batch)
            loss = outputs.loss 
        ...
        print(f"Epoch {epoch+1} - Train Loss: {train_loss} - Time: {epoch_time}")
        info_epoch[epoch] = {
        ...
        epoch_eval_time = time.time() - start_eval
        ...
        info_epoch[epoch]= {
            'Epoch eval time': epoch_eval_time,
            'Eval loss': eval_loss,
