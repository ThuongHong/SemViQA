...
from src.data.dataset.qact import load_data
from src.loss import comboLoss
...
def load_models(args):
    if args.my_model == "QACT":
        model = QACT(config=args)  
    else:
...
def main(args):
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            # Calculate loss
            loss, loss_base, crr_loss, retation_tagg_loss = loss_fn(output=output)
        ...
        print(f"Epoch {epoch+1} - epoch_training_time: {epoch_training_time} - Train Loss: {train_loss} - Base Loss: {loss_base} - Retation Tagg Loss: {retation_tagg_loss} - Balance Loss: {balance_loss if args.use_smoe else -1}")
        ...
        if torch.isnan(torch.tensor(train_loss)):
            ...
            break
        # if epoch % 4 == 0:
        #     save_path = os.path.join(args.output_dir, f"model epoch {epoch+1}")
        #     if not os.path.exists(save_path):
        #         os.makedirs(save_path)
        #     accelerator.save_state(save_path)
        #     print("Save model")
        # if prev_train_loss is not None:
        #     loss_diff = abs(train_loss - prev_train_loss)
        #     if loss_diff < args.stop_threshold:
        #         print(f"Stopping training as loss change ({loss_diff}) is below threshold ({args.stop_threshold}).")
        #         break
        ...
        print(f"Epoch {epoch+1} - epoch_eval_time: {epoch_eval_time} - Accuracy: {accuracy} - Eval Loss: {eval_loss} - F1: {eval_f1} - Kappa: {eval_kappa} - Matthews Corrcoef: {eval_matthews_corrcoef}")
...
def parse_args():
    ...
    parser.add_argument("--project", type=str, default="Find-Evidence-UIT", help="Wandb project name")
    ...
    parser.add_argument("--my_model", type=str, default="QACT", help="Model name")
