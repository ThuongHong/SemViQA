...
class Rational_Tagging(nn.Module):
    ...
    def forward(self, h_t):
        h_t = h_t.permute(1, 0, 2)  # Chuyển thành (seq_len, batch_size, hidden_size) để phù hợp với MultiheadAttention
        attn_output, _ = self.attn(h_t, h_t, h_t)  
        gated_h = torch.sigmoid(self.gate(h_t)) * attn_output  # Gated Mechanism
        h_out = self.norm(gated_h + h_t)  
        h_out = self.dropout(h_out)
        h_out = h_out.permute(1, 0, 2)  # Chuyển lại về (batch_size, seq_len, hidden_size)
